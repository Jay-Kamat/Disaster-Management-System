...
import { ShieldAlert, AlertTriangle, Globe, CheckCircle2 } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center relative overflow-hidden text-slate-200">
      ...
      <div className="glass-panel p-8 sm:p-12 rounded-3xl w-11/12 max-w-md shadow-2xl border border-slate-800/80 text-center relative z-10 flex flex-col items-center">
        ...
        {isForgotPassword ? (
          <form onSubmit={handleResetPassword} className="w-full space-y-4 mb-6">
            {successMessage && (
              <div className="w-full bg-emerald-900/30 border border-emerald-500/50 text-emerald-300 px-4 py-3 rounded-xl text-sm mb-6 flex items-start space-x-3 text-left">
                <CheckCircle2 className="w-5 h-5 shrink-0 mt-0.5" />
                <span>{successMessage}</span>
              </div>
            )}
            <div className="text-left text-xs text-slate-400 mb-2">
            ...
            <button
              ...
              className="w-full text-slate-400 hover:text-white text-sm font-semibold py-2 transition-colors"
            >
              Back to Sign In
            </button>
          </form>
